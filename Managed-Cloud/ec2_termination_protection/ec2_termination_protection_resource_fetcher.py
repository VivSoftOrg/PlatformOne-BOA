...
# from REANCloud LLC.
""" This module will fetch EC2 instances presnt in all the available regions. """
...
from common.common_constants import AWSResourceClassConstants, ManagedCloudConstants, BotoConstants, ResourceConstants


class Ec2TerminationProtectionFetcher(AbstractResourceFetcher):
    """ This class is responsible for fetching the resources """
    def __parseAndFetchInstanceDetails(self, instanceObject):
        """ This method will be used to fetch and parse ec2 instance details. """
        resourceId = instanceObject[BotoConstants.BOTO_CLIENT_AWS_EC2_INSTANCE_ID]
        ...
        try:
            ...
            for region in regions:
                ...
                for response in paginatorResponse.paginate(Filters=[
                        {
                            BotoConstants.BOTO_CLIENT_AWS_EC2_NAME: BotoConstants.BOTO_CLIENT_AWS_EC2_INSTANCE_STATE_NAME,
                            BotoConstants.BOTO_CLIENT_AWS_EC2_VALUES: [
                                BotoConstants.BOTO_CLIENT_AWS_EC2_RUNNING_STATE,
                                BotoConstants.BOTO_CLIENT_AWS_EC2_STOPPED_STATE]
                            }
                    ]):  # noqa
                    instanceResponse = CommonUtility.changeDictionaryKeysToLowerCase(response)
                    instanceReservations = []
                    if instanceResponse.__contains__(BotoConstants.BOTO_CLIENT_AWS_EC2_RESERVATION):
