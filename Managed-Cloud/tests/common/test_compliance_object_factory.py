...
class TestComplianceObjectFactory(object):

    def test_create_action_from(self):
        assert (cof.ComplianceObjectFactory.createActionFrom(eventParam))

    def test_create_event_param_from(self):
        eventParam = cof.ComplianceObjectFactory.createEventParamFrom(
            EVENTJSON,
            CONTEXT
        )
        assert (eventParam != None)

    def test_create_evaluator_from(self):
        assert (cof.ComplianceObjectFactory.createEvaluatorFrom(eventParam))

    def test_create_notification_from(self):
        notification = cof.ComplianceObjectFactory.createNotificationFrom(eventParam, "Evaluation", evaluationResult)
