...
import ec2_expiry_tag.ec2_expiry_tag_action as exptag
...
EVENTJSON = {
    ...
    "ruleParameters": "{\"expirationDays\":7,\"limit_expiration_date\":7,\"performAction\": \"True\",\"notifier\": \"sns\"}",
...
CONFIG_ITEMS_TERMINATE_INST_SPOT = {
    "resourceId": "i-01254789351",
    "state":{
        "name": "stopped"
    },
...
CONFIG_ITEMS_TERMINATE_INST_SPOT_EXCEPTION = {
    "resourceId": "i-01254789351",
    "state":{
        "name": "stopped"
    },
...
class TestEc2ExpiryTagAction(object):

    @mock_ec2
    def test_perform_action_date_terminate_instance_spot_failed(self):
        termianteInstanceIdResponse = "Error occured while terminating instance", "ami-01bf2f9b0e1a63d60"
        self._AbstractAction__eventParam = eventParam
        # self.createAMI = MagicMock(return_value=("ami-01bf2f9b0e1a63d60", ""))
        self.terminateInstance = MagicMock(return_value=termianteInstanceIdResponse)
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=CONFIG_ITEMS_TERMINATE_INST_SPOT)
        return_value = exptag.Ec2ExpiryTagAction.performAction(self, eventItem)

        assert return_value.complianceType == ComplianceConstants.NON_COMPLIANT_RESOURCE

    @mock_ec2
    def test_perform_action_date_terminate_instance_spot(self):
        termianteInstanceIdResponse = "Instance termianted and AMI created", "ami-01bf2f9b0e1a63d60"
        self._AbstractAction__eventParam = eventParam
        # self.createAMI = MagicMock(return_value=("ami-01bf2f9b0e1a63d60", ""))
        self.terminateInstance = MagicMock(return_value=termianteInstanceIdResponse)
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=CONFIG_ITEMS_TERMINATE_INST_SPOT)
    ...
    @mock_ec2
    def test_perform_action_date_terminate_instance_failed(self):
        self._AbstractAction__eventParam = eventParam
        self.terminateInstance = MagicMock(return_value=("Error occoured while modifying attribute - DisableApiTermination", ""))
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=CONFIG_ITEMS_TERMINATE_INST)
        return_value = exptag.Ec2ExpiryTagAction.performAction(self, eventItem)

        assert return_value.complianceType == ComplianceConstants.NON_COMPLIANT_RESOURCE

    @mock_ec2
    def test_perform_action_date_terminate_instance(self):
        termianteInstanceIdResponse = "Instance termianted and AMI created", "ami-01bf2f9b0e1a63d60"
        self._AbstractAction__eventParam = eventParam
        self.terminateInstance = MagicMock(return_value=termianteInstanceIdResponse)
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=CONFIG_ITEMS_TERMINATE_INST)
    ...
    def test_perform_action_date_invalid_date_failed(self):
        self._AbstractAction__eventParam = eventParam
        self.addExpirationDateTag = MagicMock(return_value=False)
    ...
    def test_perform_action_date_invalid_date(self):
        self._AbstractAction__eventParam = eventParam
        self.addExpirationDateTag = MagicMock(return_value=True)
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=CONFIG_ITEMS_INVALID_DATE)
        return_value = exptag.Ec2ExpiryTagAction.performAction(self, eventItem)
    ...
    def test_perform_action_expdate_crossed_failed(self):
        self._AbstractAction__eventParam = eventParam
        self.stopInstances = MagicMock(return_value=False)
    ...
    def test_perform_action_expdate_crossed(self):
        self._AbstractAction__eventParam = eventParam
        self.stopInstances = MagicMock(return_value=True)
        self.addExpirationDateTag = MagicMock(return_value=True)
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=CONFIG_ITEMS)
        return_value = exptag.Ec2ExpiryTagAction.performAction(self, eventItem)
        assert return_value.complianceType == ComplianceConstants.NON_COMPLIANT_RESOURCE
    ...
    def test_perform_action_with_exception(self):
        self._AbstractAction__eventParam = eventParam
        ec2 = 'ec2'
        exptag.BotoUtility.getClient = MagicMock(return_value=ec2)
