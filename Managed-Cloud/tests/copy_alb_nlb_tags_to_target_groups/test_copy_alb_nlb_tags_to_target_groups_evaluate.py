...
from copy_alb_nlb_tags_to_target_groups.copy_alb_nlb_tags_to_target_groups_evaluate import *
...
class TestCopyAlbNlbTagsToTargetGroupsEvaluate(object):
    _AbstractEvaluator__eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
        EVENT_JSON,
        CONTEXT
    )

    __albTagEvaluator = CopyAlbNlbTagsToTargetGroupsEvaluate(_AbstractEvaluator__eventParam)

    def testALBNLBResourceFetcherSuccessForComplaint(self):
        recordedMockResponsePath = PlaceboMockResponseInitializer.getEvaluatorMock('InvokingEventSuccessEvaluate')
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=CommonUtility.changeDictionaryKeysToLowerCase(CONFIG_EVENT))
    ...
    def testALBNLBResourceFetcherSuccessForNonComplaint(self):
        recordedMockResponsePath = PlaceboMockResponseInitializer.getEvaluatorMock('success_non_compliant')
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=CommonUtility.changeDictionaryKeysToLowerCase(CONFIG_EVENT))
        PlaceboMockResponseInitializer.replaying_pill(recordedMockResponsePath)
        evaluationResult = self.__albTagEvaluator.evaluate(eventItem)
        assert evaluationResult.complianceType == ComplianceConstants.NON_COMPLIANT_RESOURCE

    def testALBNLBResourceFetcherFailure(self):
        recordedMockResponsePath = PlaceboMockResponseInitializer.getEvaluatorMock('Failure')
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=CommonUtility.changeDictionaryKeysToLowerCase(CONFIG_EVENT))
        PlaceboMockResponseInitializer.replaying_pill(recordedMockResponsePath)
        evaluationResult = self.__albTagEvaluator.evaluate(eventItem)
