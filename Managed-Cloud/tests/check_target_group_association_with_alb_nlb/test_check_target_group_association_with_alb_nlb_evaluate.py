...
from check_target_group_association_with_alb_nlb.check_target_group_association_with_alb_nlb_evaluate import *
from tests.check_target_group_association_with_alb_nlb.test_check_target_group_association_with_alb_nlb_placebo_initializer import *
...
RESOURCE_ID = 'arn:aws:elasticloadbalancing:us-east-1:107339370656:targetgroup/vaibhavMNCTargetGroup/8b41b3e1be1d905f'
CONFIG_EVENT_COMPLIANT = {'targetgrouparn': 'arn:aws:elasticloadbalancing:us-east-1:107339370656:targetgroup/vaibhavMNCTargetGroup/8b41b3e1be1d905f', 'targetgroupname': 'vaibhavMNCTargetGroup', 'protocol': 'HTTP', 'port': 80, 'vpcid': 'vpc-38738c43', 'healthcheckprotocol': 'HTTP', 'healthcheckport': 'traffic-port', 'healthcheckintervalseconds': 30, 'healthchecktimeoutseconds': 5, 'healthythresholdcount': 5, 'unhealthythresholdcount': 2, 'healthcheckpath': '/', 'matcher': {'httpcode': '200'}, 'loadbalancerarns': ['arn:aws:elasticloadbalancing:us-east-1:107339370656:loadbalancer/app/mncTestVaibhav/197421797bfcde8f'], 'targettype': 'instance', 'region': 'us-east-1'}
CONFIG_EVENT_NON_COMPLIANT = {'targetgrouparn': 'arn:aws:elasticloadbalancing:us-east-1:107339370656:targetgroup/vaibhavMNCTargetGroup/8b41b3e1be1d905f', 'targetgroupname': 'vaibhavMNCTargetGroup', 'protocol': 'HTTP', 'port': 80, 'vpcid': 'vpc-38738c43', 'healthcheckprotocol': 'HTTP', 'healthcheckport': 'traffic-port', 'healthcheckintervalseconds': 30, 'healthchecktimeoutseconds': 5, 'healthythresholdcount': 5, 'unhealthythresholdcount': 2, 'healthcheckpath': '/', 'matcher': {'httpcode': '200'}, 'loadbalancerarns': [], 'targettype': 'instance', 'region': 'us-east-1'}
CONFIG_EVENT_EXCEPTION = {'targetgrouparn': 'arn:aws:elasticloadbalancing:us-east-1:107339370656:targetgroup/vaibhavMNCTargetGroup/8b41b3e1be1d905f', 'targetgroupname': 'vaibhavMNCTargetGroup', 'protocol': 'HTTP', 'port': 80, 'vpcid': 'vpc-38738c43', 'healthcheckprotocol': 'HTTP', 'healthcheckport': 'traffic-port', 'healthcheckintervalseconds': 30, 'healthchecktimeoutseconds': 5, 'healthythresholdcount': 5, 'unhealthythresholdcount': 2, 'healthcheckpath': '/', 'matcher': {'httpcode': '200'}, 'targettype': 'instance', 'region': 'us-east-1'}


class TestCheckTargetGroupAssociationWithAlbNlbEvaluate(object):
    _AbstractEvaluator__eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
        EVENT_JSON,
        CONTEXT
    )

    __albTagEvaluator = CheckTargetGroupAssociationWithAlbNlbEvaluate(_AbstractEvaluator__eventParam)

    def testCheckTargetGroupAssociationWithAlbNlbEvaluateForComplaint(self):
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=CONFIG_EVENT_COMPLIANT)
        evaluationResult = self.__albTagEvaluator.evaluate(eventItem)
        assert evaluationResult.complianceType == ComplianceConstants.COMPLIANT_RESOURCE

    def testCheckTargetGroupAssociationWithAlbNlbEvaluateForNonComplaint(self):
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=CONFIG_EVENT_NON_COMPLIANT)
        evaluationResult = self.__albTagEvaluator.evaluate(eventItem)
        assert evaluationResult.complianceType == ComplianceConstants.NON_COMPLIANT_RESOURCE

    def testCheckTargetGroupAssociationWithAlbNlbEvaluateForException(self):
        recordedMockResponsePath = PlaceboMockResponseInitializer.getEvaluatorMock('exception')
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=CONFIG_EVENT_EXCEPTION)
