  ...
  configmap_data = merge( include.terragrunt-vars.locals.configmap_data,
    {
      ...
      batcave_bucket_gitlab = one(keys(dependency.gitlab_storage.outputs.s3_buckets))
      # In the dev environment there is no separate runner cache bucket.
      batcave_bucket_gitlab_runner_cache = one(keys(dependency.gitlab_storage.outputs.s3_buckets))
    }
  )
