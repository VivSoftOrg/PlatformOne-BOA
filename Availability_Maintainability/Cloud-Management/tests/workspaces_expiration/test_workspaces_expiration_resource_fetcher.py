...
from workspaces_expiration.workspaces_expiration_resource_fetcher import *
...
class TestWorkSpaceExpirationFetcher(unittest.TestCase):
    __eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
        EVENT_JSON,
        CONTEXT
    )

    __workSpaceFetcher = WorkSpacesResourceFetcher(__eventParam)

    def testWorkSpaceResourceFetcher(self):
        recordedMockResponsePath = PlaceboMockResponseInitializer.getResourceFetcherMock('fetch_workspaces')
        PlaceboMockResponseInitializer.replaying_pill(recordedMockResponsePath)
        return_value = self.__workSpaceFetcher.resourceFetcher()
        assert bool(return_value) == True

    def testWorkSpaceResourceFetcherNoData(self):
        recordedMockResponsePath = PlaceboMockResponseInitializer.getResourceFetcherMock('fetch_workspaces_blank_response')
        PlaceboMockResponseInitializer.replaying_pill(recordedMockResponsePath)
        return_value = self.__workSpaceFetcher.resourceFetcher()
