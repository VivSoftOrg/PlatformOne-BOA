...
import copy_ebs_tags_to_snapshots.copy_ebs_tags_to_snapshots_action as copyEBS
...
class TestCopyEbsTagsToSnapshotsAction(object):
    ...
    __copyEBS = copyEBS.CopyEbsTagsToSnapshotsAction(__eventParam)

    def testCopyEbsTagToSnapshotsActionSuccess(self):
        evaluationResult = EvaluationResult()
        configItems = {'CopyTags': [
            {
                'Id': 'Test-12342',
                'tagsToCopy': [
                    {
        ...
        ec2client.create_tags = MagicMock(return_value=True)
        ...
        return_value = self.__copyEBS.performAction(eventItem)
        assert return_value.complianceType == ComplianceConstants.COMPLIANT_RESOURCE

    def testCopyEbsTagToSnapshotsActionFailure(self):
        evaluationResult = EvaluationResult()
        configItems = {}
        ec2client = boto3.client(BotoConstants.BOTO_CLIENT_AWS_EC2, region_name=self.__regions[0])
        copyEBS.BotoUtility.getClient = MagicMock(return_value=ec2client)
