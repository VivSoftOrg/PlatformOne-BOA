...
from unencrypted_snapshot.unencrypted_snapshot_evaluate import *
...
class TestUnencryptedSnapshotEvaluate(object):
    ...
    __unencryptedSnapshotEvaluator = UnencryptedSnapshotEvaluate(_AbstractEvaluator__eventParam)

    def testEvaluateForSnapshotVolumeNotEncrypted(self):
        configItem = {
            "encrypted": False
        }
        ...
        evaluationResult = self.__unencryptedSnapshotEvaluator.evaluate(eventItem)
        assert evaluationResult.complianceType == ComplianceConstants.NON_COMPLIANT_RESOURCE

    def testEvaluateForSnapshotVolumeEncrypted(self):
        configItem = {
            "encrypted": True
        }
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=configItem)
