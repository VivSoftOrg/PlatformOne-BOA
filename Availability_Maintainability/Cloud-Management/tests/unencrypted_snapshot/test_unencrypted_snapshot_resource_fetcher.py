...
sys.path.append('../../')
import boto3
from unencrypted_snapshot.unencrypted_snapshot_resource_fetcher import *
...
class TestUnencryptedSnapshotFetcher(unittest.TestCase):
    ...
    __mockResponse = {'Snapshots': [
            {'Encrypted': False, 'SnapshotId': "snap-adasdq3q423", 'VolumeId': 'vol-0a8dd6aa5bc3ddd57', 'Tags': [
                {'Key': 'Owner', 'Value': 'vaibhav'
                 },
    ...
    __snapshotResourceFetcher = UnencryptedSnapshotResourceFetcher(__eventParam)
    ...
    def testResourceFetcher(self):
        ...
        ec2Client.describe_snapshots = MagicMock(return_value=self.__mockResponse)
        return_value = self.__snapshotResourceFetcher.resourceFetcher()
    ...
    def testResourceFetcherFailure(self):
        ec2Client = boto3.client(BotoConstants.BOTO_CLIENT_AWS_EC2, region_name=self.__awsRegion)
        BotoUtility.getClient = MagicMock(return_value=ec2Client)
        ec2Client.describe_snapshots = MagicMock(side_effect=KeyError("Failed to fetch resource details"))
        return_value = UnencryptedSnapshotResourceFetcher.resourceFetcher(self)
