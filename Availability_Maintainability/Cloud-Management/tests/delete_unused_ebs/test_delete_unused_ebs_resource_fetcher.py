...
from delete_unused_ebs.delete_unused_ebs_resource_fetcher import *
...
class TestDeleteUnusedEBSFetcher(unittest.TestCase):
    __eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
        EVENT_JSON,
        CONTEXT
    )

    __ebsVolumesFetcher = DeleteUnusedEBSResourceFetcher(__eventParam)

    def testEbsVolumesResourceFetcher(self):
        recordedMockResponsePath = PlaceboMockResponseInitializer.getResourceFetcherMock('fetch_ebs')
        PlaceboMockResponseInitializer.replaying_pill(recordedMockResponsePath)
        return_value = self.__ebsVolumesFetcher.resourceFetcher()
        assert bool(return_value) == True

    def testEbsVolumesResourceFetcherNoData(self):
        recordedMockResponsePath = PlaceboMockResponseInitializer.getResourceFetcherMock('fetch_ebs_blank_response')
        PlaceboMockResponseInitializer.replaying_pill(recordedMockResponsePath)
        return_value = self.__ebsVolumesFetcher.resourceFetcher()
