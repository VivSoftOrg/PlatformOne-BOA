...
from check_target_group_association_with_alb_nlb.check_target_group_association_with_alb_nlb_resource_fetcher import *
...
class TestCheckTargetGroupAssociationWithAlbNlbResourceFetcher(unittest.TestCase):
    __eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
        EVENT_JSON,
        CONTEXT
    )

    __targetGroupFetcher = CheckTargetGroupAssociationWithAlbNlbResourceFetcher(__eventParam)

    def testALBLBResourceFetcherSuccess(self):
        recordedMockResponsePath = PlaceboMockResponseInitializer.getResourceFetcherMock('Success')
        PlaceboMockResponseInitializer.replaying_pill(recordedMockResponsePath)
        return_value = self.__targetGroupFetcher.resourceFetcher()
        assert bool(return_value) == True

    def testALBNLBResourceFetcherFailure(self):
        recordedMockResponsePath = PlaceboMockResponseInitializer.getResourceFetcherMock('Failure')
        PlaceboMockResponseInitializer.replaying_pill(recordedMockResponsePath)
        return_value = self.__targetGroupFetcher.resourceFetcher()
