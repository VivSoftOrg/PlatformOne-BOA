...
from check_target_group_association_with_alb_nlb.check_target_group_association_with_alb_nlb_action import *
...
class TestCheckTargetGroupAssociationWithAlbNlbAction:
    _AbstractAction__eventParam = ComplianceObjectFactory.createEventParamFrom(
        EVENT_JSON,
        CONTEXT
        )

    __targetGroupDelete = CheckTargetGroupAssociationWithAlbNlbAction(_AbstractAction__eventParam)

    def testCheckTargetGroupAssociationWithAlbNlbSucess(self):
        ...
        return_value = self.__targetGroupDelete.performAction(eventItem)
        assert return_value.complianceType == ComplianceConstants.COMPLIANCE_NOT_APPLICABLE

    def testCheckTargetGroupAssociationWithAlbNlbFailure(self):
        recordedMockResponsePath = PlaceboMockResponseInitializer.getActionMock('action_failure')
        PlaceboMockResponseInitializer.replaying_pill(recordedMockResponsePath)
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=CONFIG_EVENT)
        return_value = self.__targetGroupDelete.performAction(eventItem)
        assert return_value.complianceType == ComplianceConstants.NON_COMPLIANT_RESOURCE

    def testCheckTargetGroupAssociationWithAlbNlbException(self):
        recordedMockResponsePath = PlaceboMockResponseInitializer.getActionMock('action_exception')
        PlaceboMockResponseInitializer.replaying_pill(recordedMockResponsePath)
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=CONFIG_EVENT)
        return_value = self.__targetGroupDelete.performAction(eventItem)
