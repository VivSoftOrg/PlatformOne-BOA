...
import ec2_exposed_instances.ec2_exposed_instances_resource_fetcher as exposedInstancesFetcher
...
class TestExposedInstancesFetcher(object):
    ...
    def testResourceFetcher(self):
        ...
        exposedInstancesFetcher.BotoUtility.getClient = MagicMock(return_value=ec2Client)
        ec2Client.describe_instances = MagicMock(return_value=self.__mockResponse)
        self._EC2ExposedInstancesResourceFetcher__parseAndFetchInstanceDetails = MagicMock(return_value=True)
        return_value = exposedInstancesFetcher.EC2ExposedInstancesResourceFetcher.resourceFetcher(self)
    ...
    @mock_ec2
    def testResourceFetcherValueError(self):
        self._AbstractResourceFetcher__eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
            EVENT_JSON,
            CONTEXT
        )
        eventItem = complianceobjectfactory.ComplianceObjectFactory.createEventItemFrom(
            EVENT_JSON,
            CONTEXT,
            self._AbstractResourceFetcher__eventParam
        )
        ec2Client = boto3.client(BotoConstants.BOTO_CLIENT_AWS_EC2, region_name=self.__awsRegion)
        exposedInstancesFetcher.BotoUtility.getClient = MagicMock(return_value=ec2Client)
        ec2Client.describe_instances = MagicMock(side_effect=ValueError("Value not found error"))
        return_value = exposedInstancesFetcher.EC2ExposedInstancesResourceFetcher.resourceFetcher(self)
        assert bool(return_value) == False

    @mock_ec2
    def testResourceFetcherAttributeError(self):
        self._AbstractResourceFetcher__eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
            EVENT_JSON,
            CONTEXT
        )
        ...
        ec2Client = boto3.client(BotoConstants.BOTO_CLIENT_AWS_EC2, region_name=self.__awsRegion)
        exposedInstancesFetcher.BotoUtility.getClient = MagicMock(return_value=ec2Client)
        ec2Client.describe_instances = MagicMock(side_effect=AttributeError("Attribute not found error"))
        return_value = exposedInstancesFetcher.EC2ExposedInstancesResourceFetcher.resourceFetcher(self)
        assert bool(return_value) == False

    @mock_ec2
    def testResourceFetcherKeyError(self):
        self._AbstractResourceFetcher__eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
            EVENT_JSON,
            CONTEXT
        )
        ...
        ec2Client = boto3.client(BotoConstants.BOTO_CLIENT_AWS_EC2, region_name=self.__awsRegion)
        exposedInstancesFetcher.BotoUtility.getClient = MagicMock(return_value=ec2Client)
        ec2Client.describe_instances = MagicMock(side_effect=KeyError("Key not found error"))
        return_value = exposedInstancesFetcher.EC2ExposedInstancesResourceFetcher.resourceFetcher(self)
        assert bool(return_value) == False

    @mock_ec2
    def testResourceFetcherExceptionError(self):
        self._AbstractResourceFetcher__eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
            EVENT_JSON,
            CONTEXT
        )
        ...
        ec2Client = boto3.client(BotoConstants.BOTO_CLIENT_AWS_EC2, region_name=self.__awsRegion)
        exposedInstancesFetcher.BotoUtility.getClient = MagicMock(return_value=ec2Client)
        ec2Client.describe_instances = MagicMock(side_effect=Exception("Resource not found error"))
        return_value = exposedInstancesFetcher.EC2ExposedInstancesResourceFetcher.resourceFetcher(self)
        assert bool(return_value) == False

    @mock_ec2
    def testResourceFetcherClientError(self):
        self._AbstractResourceFetcher__eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
            EVENT_JSON,
            CONTEXT
        )
    ...
    @mock_ec2
    def testResourceFetcherTypeError(self):
        self._AbstractResourceFetcher__eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
            EVENT_JSON,
            CONTEXT
        )
        ...
        ec2Client = boto3.client(BotoConstants.BOTO_CLIENT_AWS_EC2, region_name=self.__awsRegion)
        exposedInstancesFetcher.BotoUtility.getClient = MagicMock(return_value=ec2Client)
        ec2Client.describe_instances = MagicMock(side_effect=TypeError("Tyoe not found error"))
        return_value = exposedInstancesFetcher.EC2ExposedInstancesResourceFetcher.resourceFetcher(self)
        assert bool(return_value) == False

    @mock_ec2
    def testResourceFetcherNameError(self):
        self._AbstractResourceFetcher__eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
            EVENT_JSON,
            CONTEXT
        )
        ...
        ec2Client = boto3.client(BotoConstants.BOTO_CLIENT_AWS_EC2, region_name=self.__awsRegion)
        exposedInstancesFetcher.BotoUtility.getClient = MagicMock(return_value=ec2Client)
        ec2Client.describe_instances = MagicMock(side_effect=NameError("Name not found error"))
        return_value = exposedInstancesFetcher.EC2ExposedInstancesResourceFetcher.resourceFetcher(self)
        assert bool(return_value) == False
