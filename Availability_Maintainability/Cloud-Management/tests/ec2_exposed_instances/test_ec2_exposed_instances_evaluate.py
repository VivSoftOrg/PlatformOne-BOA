...
import ec2_exposed_instances.ec2_exposed_instances_evaluate as exposedInstances
...
class TestExposedInstancesEvaluate(unittest.TestCase):
    ...
    def initializePlacebo(self, mockType):
        PLACEBO_DIR = 'placebo_recorded_responses'
        dir_path = os.path.dirname(os.path.realpath(__file__))
        mock_path = dir_path + '/' + PLACEBO_DIR + '/' + mockType
        boto3.setup_default_session()
        session = boto3.DEFAULT_SESSION
        self.pill = placebo.attach(session, data_path= os.path.abspath(mock_path))
        self.pill.playback()

    def testEvaluateForNotApplicableResourceType(self):
        ...
        self._EC2ExposedInstancesEvaluate__applicableResources = [AWSResourceClassConstants.EC2_INSTANCE]
        evaluationResult = exposedInstances.EC2ExposedInstancesEvaluate.evaluate(self, eventItem[0])
        assert evaluationResult.complianceType == ComplianceConstants.COMPLIANCE_NOT_APPLICABLE

    def testEvaluateForTerminatedResource(self):
        self._AbstractEvaluator__eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
            EVENT_JSON_TERMINATED_RESOURCE,
            CONTEXT
        )
    ...
    def testEvaluateForResourceWithoutSecGrps(self):
        self._AbstractEvaluator__eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
            EVENT_JSON_WITHOUT_SEC_GRPS,
            CONTEXT
        )
    ...
    @mock_ec2
    def testEvaluateForResourceWithViolation(self):
        self._AbstractEvaluator__eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
            EVENT_JSON,
            CONTEXT
        )
        ...
        self.initializePlacebo('evaluate_non_compliant_resources')
        self._EC2ExposedInstancesEvaluate__applicableResources = [AWSResourceClassConstants.EC2_INSTANCE]
        evaluationResult = exposedInstances.EC2ExposedInstancesEvaluate(self._AbstractEvaluator__eventParam).evaluate(eventItem[0])
        assert evaluationResult.complianceType == ComplianceConstants.NON_COMPLIANT_RESOURCE

    @mock_ec2
    def testEvaluateWithoutViolation(self):
        self._AbstractEvaluator__eventParam = complianceobjectfactory.ComplianceObjectFactory.createEventParamFrom(
            EVENT_JSON,
            CONTEXT
        )
        ...
        self.initializePlacebo('evaluate_compliant_resources')
        self._EC2ExposedInstancesEvaluate__applicableResources = [AWSResourceClassConstants.EC2_INSTANCE]
        evaluationResult = exposedInstances.EC2ExposedInstancesEvaluate(self._AbstractEvaluator__eventParam).evaluate(eventItem[0])
        assert evaluationResult.complianceType == ComplianceConstants.COMPLIANT_RESOURCE
