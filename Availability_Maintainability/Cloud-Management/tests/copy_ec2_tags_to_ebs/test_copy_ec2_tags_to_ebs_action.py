...
# from REANCloud LLC.
""" This module test the action class of the rule """
import sys

sys.path.append('../../')

import copy_ec2_tags_to_ebs.copy_ec2_tags_to_ebs_action as copyTagsAction
...
class TestEnvTagAction(object):
    """ This class is used to test the action class of the rule """
    ...
    __configItems = {
        ...
        'VolumesTags': [{
            ...
            'tagsToCopy': [
                ...
                {
                    "Key": "Owner",
                    "Value": "devendra.suthar"
                }
            ]
        }]
    }

    __copyTagsAction = copyTagsAction.CopyEc2TagsToEbsAction(__eventParam)

    def testPerformActionAddTagsToVolumeSuccess(self):
        recordedMockResponsePath = placeboInitializer.PlaceboMockResponseInitializer.getActionMock(
            'success')
        ...
        return_value = self.__copyTagsAction.performAction(eventItem)
        assert return_value.complianceType == ComplianceConstants.COMPLIANT_RESOURCE

    def testPerformActionAddTagsToVolumeException(self):
        recordedMockResponsePath = placeboInitializer.PlaceboMockResponseInitializer.getActionMock(
            'exception')
        placeboInitializer.PlaceboMockResponseInitializer.replaying_pill(recordedMockResponsePath)
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID,
