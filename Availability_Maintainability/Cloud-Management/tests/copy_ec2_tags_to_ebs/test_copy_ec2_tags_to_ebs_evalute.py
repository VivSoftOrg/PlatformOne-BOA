...
# from REANCloud LLC.
""" This module is test case module for evaluator class """
import sys

sys.path.append('../../')

import copy_ec2_tags_to_ebs.copy_ec2_tags_to_ebs_evaluate as evaluator
import tests.copy_ec2_tags_to_ebs.test_copy_ec2_tags_to_ebs_placebo_initializer as placeboInitializer
...
EVENT_JSON = {
    ...
    "ruleParameters": "{\"actionName\": \"CopyEc2TagsToEbsAction\", \"evaluateName\": \"CopyEc2TagsToEbsEvaluate\", \"resourceFetcherName\" :\"CopyEc2TagsToEbsResourceFetcher\",\"tagsToCopy\": \"Name,Owner,Environment\", \n \"notifier\":\"email\",\n \"toEmail\": \"devendra.suthar@reancloud.com\",\n \"ccEmail\": \"devendra.suthar@reancloud.com\",\n  \"performAction\": \"True\"}",
...
RESOURCE_ID = "DummyInstance"
RESOURCE_TYPE = "AWS::EC2::Instance"

class TestEnvTagEvaluate(object):
    """ This class test the evaluate class of the ec2 environment tag check """
    ...
    __copyTagsEvaluate = evaluator.CopyEc2TagsToEbsEvaluate(__eventParam)

    __configItems = {
        'resourceName': 'DummyInstance',
        'region': 'us-east-1'
    }

    def testEvaluateVolumeHaveAllTags(self):
        recordedMockResponsePath = placeboInitializer.PlaceboMockResponseInitializer.getEvaluatorMock('volume_have_all_tags')
        placeboInitializer.PlaceboMockResponseInitializer.replaying_pill(recordedMockResponsePath)
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID,
                                                                         resourceType=RESOURCE_TYPE,
                                                                         configItems=self.__configItems)
        return_value = self.__copyTagsEvaluate.evaluate(eventItem)
        assert return_value.complianceType == ComplianceConstants.COMPLIANT_RESOURCE

    def testEvaluateVolumeDoesNotHaveAllTags(self):
        recordedMockResponsePath = placeboInitializer.PlaceboMockResponseInitializer.getEvaluatorMock('volume_does_not_have_tags')
        placeboInitializer.PlaceboMockResponseInitializer.replaying_pill(recordedMockResponsePath)
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID,
                                                                         resourceType=RESOURCE_TYPE,
                                                                         configItems=self.__configItems)
