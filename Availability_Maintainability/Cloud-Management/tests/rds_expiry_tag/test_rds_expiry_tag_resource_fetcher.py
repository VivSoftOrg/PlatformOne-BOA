...
import rds_expiry_tag.rds_expiry_tag_resource_fetcher as resource_fetcher
...
class TestRdsExpiryTagAction(object):
    """ This class is used to test resource fetcher for RDS Expiry Tag """
    @mock_rds
    def test_resource_fetcher(self):
        """ This test for resource fetcher with no exception """
        self._AbstractResourceFetcher__eventParam = eventParam

        rds = boto3.client('rds', 'us-east-1')

        resource_fetcher.BotoUtility.getClient = MagicMock(return_value=rds)
        return_value = resource_fetcher.RdsExpiryTagFetcher.resourceFetcher(self)

        assert return_value != False

    def test_resource_fetcher_with_exception(self):
        """ This test for resource fetcher with exception """
        self._AbstractResourceFetcher__eventParam = eventParam
        rds = 'rds'
        resource_fetcher.BotoUtility.getClient = MagicMock(return_value=rds)
