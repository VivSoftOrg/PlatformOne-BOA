...
dependency "rapidfort_storage" {
  config_path = "../apps/rapidfort/storage"
  mock_outputs = {
    s3_buckets = {}
  }
}
...
inputs = {
  ...
  # Create a second ALB in the shared subnet to open access
  # to services across CMSCloud (AWS)
  ...
  s3_bucket_access_grants = concat(
    ...
    keys(dependency.rapidfort_storage.outputs.s3_buckets),
