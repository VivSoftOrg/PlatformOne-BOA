...
from common.common_constants import BotoConstants, ComplianceConstants, ResourceConstants, TagsConstants, ManagedCloudConstants
...
class CheckRDSRequiredTagValuesAction(AbstractAction):
    ...
    def performAction(self, eventItem):
        ...
        try:
            ...
            if RdsUtils.isRdsMultiAZ(eventItem):
                ...
                self._AbstractAction__recommendationMessage = _(multiAZ_Message)
                self._AbstractAction__actionMessage = _("None")
                LoggerUtility.logInfo(multiAZ_Message)
                evaluationResult.complianceType = ComplianceConstants.NON_COMPLIANT_RESOURCE
                evaluationResult.annotation = "RDS instance does not have required tags."
                return evaluationResult
            elif instanceStatus == RdsConstants.RDS_AVAILABLE_STATE_REFERENCE and \
                    eventItem.configItems['engine'] != RdsConstants.RDS_AURORA_TYPE:   # Aurora doesn't support start and stop
                ...
                evaluationResult.complianceType = ComplianceConstants.NON_COMPLIANT_RESOURCE
                evaluationResult.annotation = _("RDS instance does not have required tags.")
                LoggerUtility.logInfo(actionMessage)
                self._AbstractAction__actionMessage = "RDS instance has been stopped."
                return evaluationResult

            expirationDateLimit = int(self._AbstractAction__eventParam.configParam[ResourceConstants.RESOURCE_VALIDITY_RULE_PARAM])
            resourceValidityState = DateValidationUtil.updateEventActionBasedOnDate(
                eventItem,
                expirationDateLimit,
                TagsConstants.REQUIRED_TAGS_EXPIRATION_DATE_TAG_KEY)
            ...
            if resourceValidityState in [ResourceConstants.RESOURCE_VALIDITY_EXPIRED]:
                ...
                evaluationResult.annotation = _("Resource does not have any valid tags present.")
            elif resourceValidityState in RdsConstants.RDS_MISSING_OR_INVALID_EXPIRATION_TAG:
                defaultValidity = int(self._AbstractAction__eventParam.configParam[ResourceConstants.RESOURCE_VALIDITY_RULE_PARAM])
                response = RdsUtils.addExpirationDateTag(rdsClient, defaultValidity, eventItem, TagsConstants.REQUIRED_TAGS_EXPIRATION_DATE_TAG_KEY)
                actionMessage += str(eventItem.configItems['addExpirationDateMessage'])
                self._AbstractAction__actionMessage = _(actionMessage)
                evaluationResult.complianceType = ComplianceConstants.NON_COMPLIANT_RESOURCE
                evaluationResult.annotation = _("Resource does not have valid tags, adding expiration date.")
            elif resourceValidityState in RdsConstants.RDS_NOT_EXPIRED:
                recommendationMessage += RdsConstants.RDS_EXPIRATION_REMAINING_DAYS.format(instanceIdentifier, validityRemaining)
                response = self._AbstractAction__recommendationMessage = _(recommendationMessage)
                self._AbstractAction__actionMessage = _("None")
                evaluationResult.complianceType = ComplianceConstants.NON_COMPLIANT_RESOURCE
                evaluationResult.annotation = _(response)
                LoggerUtility.logInfo(recommendationMessage)
            ...
            errorMessage = "Error occured while deleting user profile. {}".format(e)
