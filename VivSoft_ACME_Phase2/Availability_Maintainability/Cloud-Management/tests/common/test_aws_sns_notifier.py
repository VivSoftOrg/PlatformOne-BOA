...
import common.aws_sns_notifier as asn
...
os.environ["queueName"] = "testQueue" 

class TestAWSSNSNotifier(object):
    @mock_sns
    def test_aws_sns_notifier(self):
        messageParam = NotificationMessageParam()
        messageParam.message = "Evalutaion"
        evaluationMessage = messageParam

        evaluationResult = EvaluationResult()
        evaluationResult.complianceType = 'NON_COMPLIANT'
        notification = Notification(1, 'sns', evaluationMessage, evaluationResult)

        sns = boto3.client('sns', region_name='us-east-1')
        asn.boto3.client = MagicMock(return_value = sns)
        sns.publish = MagicMock(return_value = True)

        aws_sns_notifier = asn.AWSSNSNotifier.notify(notification)
        assert aws_sns_notifier == True

    @mock_sns
    def test_aws_sns_notifier_with_action_message(self):
        messageParam = NotificationMessageParam()
        messageParam.message = "Evalutaion"
        evaluationMessage = messageParam
        ...
        action = ComplianceObjectFactory.createActionFrom(
            eventParam
        )

        messageParamAction = NotificationMessageParam()
        messageParamAction.message = "Action"
        notification.actionMessage = messageParamAction
        aws_sns_notifier = asn.AWSSNSNotifier.notify(notification)
        assert aws_sns_notifier == True
