...
import ec2_termination_protection.ec2_termination_protection_action as instanceAction
...
class TestEc2NoshutdownTagAction(object):
    ...
    __ec2TerminationProtection = instanceAction.Ec2TerminationProtectionAction(__eventParam)
    ...
    @mock_ec2
    def test_ec2_termination_protection_success(self):
        ec2 = boto3.resource('ec2', 'us-east-1')
        vpc = ec2.create_vpc(CidrBlock='192.168.0.0/16')
        vpc.create_tags(Tags=[{"Key": "Name", "Value": "default_vpc"}])
        ig = ec2.create_internet_gateway()
        ...
        RESOURCE_ID = instances[0].instance_id
        CONFIG_ITEMS['region'] = 'us-east-1'
        instanceAction.BotoUtility.getClient = MagicMock(return_value=ec2)
        eventItem = ComplianceObjectFactory.createAWSConfigEventItemfrom(resourceId=RESOURCE_ID, resourceType=RESOURCE_TYPE, configItems=CONFIG_ITEMS)
        ec2.modify_instance_attribute = MagicMock(return_value=self.__mockInstanceResponse)
        evaluationResult = self.__ec2TerminationProtection.performAction(eventItem)
        assert evaluationResult.complianceType == ComplianceConstants.COMPLIANT_RESOURCE

    @mock_ec2
    def test_ec2_termination_protection_failure(self):
        eventItem = self.__getEventItem()
        ec2 = boto3.client('ec2', 'us-east-1')
        instanceAction.BotoUtility.getClient = MagicMock(return_value=ec2)
        ec2.modify_instance_attribute = MagicMock(return_value=self.__mockInstanceResponse_fail)
        evaluationResult = self.__ec2TerminationProtection.performAction(eventItem)
        assert evaluationResult.complianceType == ComplianceConstants.NON_COMPLIANT_RESOURCE

    @mock_ec2
    def test_ec2_termination_protection_exception(self):
        eventItem = self.__getEventItem()
        ec2 = boto3.client('ec2', 'us-east-1')
        instanceAction.BotoUtility.getClient = MagicMock(return_value=ec2)
        ec2.modify_instance_attribute = MagicMock(side_effect=KeyError("Mock Exception: Key not found error"))
        evaluationResult = self.__ec2TerminationProtection.performAction(eventItem)
        assert evaluationResult.complianceType == ComplianceConstants.MNC_ACTION_EXCEPTION

    @mock_ec2
    def test_ec2_termination_protection_exception_value_error(self):
        eventItem = self.__getEventItem()
        ec2 = boto3.client('ec2', 'us-east-1')
        instanceAction.BotoUtility.getClient = MagicMock(return_value=ec2)
        ec2.modify_instance_attribute = MagicMock(side_effect=ValueError("Mock Exception: value not found error"))
        evaluationResult = self.__ec2TerminationProtection.performAction(eventItem)
        assert evaluationResult.complianceType == ComplianceConstants.MNC_ACTION_EXCEPTION

    @mock_ec2
    def test_ec2_termination_protection_exception_error(self):
        eventItem = self.__getEventItem()
        ec2=boto3.client('ec2', 'us-east-1')
        instanceAction.BotoUtility.getClient = MagicMock(return_value=ec2)
        ec2.modify_instance_attribute = MagicMock(side_effect=Exception("Mock Exception: Incorrect Syntax"))
        evaluationResult = self.__ec2TerminationProtection.performAction(eventItem)
        assert evaluationResult.complianceType == ComplianceConstants.MNC_ACTION_EXCEPTION

    @mock_ec2
    def test_ec2_termination_protection_exception_attribute_error(self):
        eventItem = self.__getEventItem()
        ec2=boto3.client('ec2', 'us-east-1')
        instanceAction.BotoUtility.getClient = MagicMock(return_value=ec2)
        ec2.modify_instance_attribute = MagicMock(side_effect=AttributeError("Mock Exception: Attribute not found error"))
        evaluationResult = self.__ec2TerminationProtection.performAction(eventItem)
        assert evaluationResult.complianceType == ComplianceConstants.MNC_ACTION_EXCEPTION

    @mock_ec2
    def test_ec2_termination_protection_exception_client_error(self):
        eventItem = self.__getEventItem()
        ec2 = boto3.client('ec2', 'us-east-1')
        instanceAction.BotoUtility.getClient = MagicMock(return_value=ec2)
        ec2.modify_instance_attribute = MagicMock(side_effect=ClientError(self.__errorResponse,"AssumeRole"))
        evaluationResult = self.__ec2TerminationProtection.performAction(eventItem)
        assert evaluationResult.complianceType == ComplianceConstants.MNC_ACTION_EXCEPTION