...
import ec2_termination_protection.ec2_termination_protection_resource_fetcher as instanceFetcher
import time
...
class TestEc2TerminationProtection(object):
    ...
    __ec2TerminationProtectionFetcher = instanceFetcher.Ec2TerminationProtectionFetcher(__eventParam)
    ...
    def test_resource_fetcher_no_data(self):
        ...
        instanceFetcher.BotoUtility.getClient = MagicMock(return_value=ec2Client)
        ec2Client.describe_instances = MagicMock(return_value = self.__blankMockResponse)
        return_value = self.__ec2TerminationProtectionFetcher.resourceFetcher()
        assert bool(return_value) == False

    @mock_ec2
    def test_resource_fetcher_failure_keyerror(self):
        self._AbstractResourceFetcher__eventParam = self.__eventParam
        ec2Client = boto3.client(BotoConstants.BOTO_CLIENT_AWS_EC2, self.__regions[0])
        instanceFetcher.BotoUtility.getClient = MagicMock(return_value=ec2Client)
    ...
    @mock_ec2
    def test_resource_fetcher_failure_syntaxerror(self):
        self._AbstractResourceFetcher__eventParam = self.__eventParam
        ec2Client = boto3.client(BotoConstants.BOTO_CLIENT_AWS_EC2, self.__regions[0])
        instanceFetcher.BotoUtility.getClient = MagicMock(return_value=ec2Client)
        ec2Client.describe_instances = MagicMock(side_effect=SyntaxError("Incorrect syntax!"))
        return_value = self.__ec2TerminationProtectionFetcher.resourceFetcher()
        assert bool(return_value) == False
