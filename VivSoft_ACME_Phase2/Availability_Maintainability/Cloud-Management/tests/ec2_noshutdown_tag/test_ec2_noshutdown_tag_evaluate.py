...
import os
sys.path.append('../../')
import ec2_noshutdown_tag.ec2_noshutdown_tag_evaluate as tagEvaluate
...
EVENT_JSON = {
    ...
    "ruleParameters": "{\"instanceIdsToSkip\":\"i-0d1a6a41f136b426f,i-08ee3e0a5894f6b59,i-041890c697d58ed56\",\"performAction\": \"True\",\"notifier\": \"sns\"}",
...
class TestEc2NoshutdownTagEvaluate(object):
    ...
    __ec2NoshutdownTagEvaluate = tagEvaluate.Ec2NoshutdownTagEvaluate(__eventParam)
    ...
    @mock_ec2
    def test_ec2_noshutdown_tag_noncompliant(self):
        ...
        ec2.create_tags(Resources=[instances[0].instance_id], Tags=[{'Key': 'NoShutdown', 'Value': 'Yes'}])
        ec2 = boto3.client(NoShutDownTagConstants.EC2_KEYWORD, 'us-east-1')
        ...
        CONFIG_ITEMS = CommonUtility.changeDictionaryKeysToLowerCase(response[NoShutDownTagConstants.EC2_RESERVATION_KEYWORD][0][NoShutDownTagConstants.EC2_INSTANCES_KEYWORD][0])
        ...
        evaluationResult = self.__ec2NoshutdownTagEvaluate.evaluate(eventItem)
        assert evaluationResult.complianceType == ComplianceConstants.NON_COMPLIANT_RESOURCE

    @mock_ec2
    def test_ec2_noshutdown_tag_compliant(self):
    ...
    @mock_ec2
    def test_ec2_noshutdown_tag_without_tags(self):
